...
/**
 * This class implements the conflict with table.
 *
 * @since 1.0
 */
// ------------------------------------------------------------------------
...
public enum ConflictWithTableModel implements ITableModel {
	...
	HEADLINE(Messages.ConfictWithTableDefinition_headline, 200, true, true, SWT.LEFT);
	...
	private ConflictWithTableModel(String aName, int aWidth, boolean aResize, boolean aMove, int align) {
		fHeader = aName;
		fwidth = aWidth;
		fResize = aResize;
		fMoveable = aMove;
	...
	public int getMinimumWidth() {
		...
		for (int index = 0; index < ConflictWithTableModel.values().length; index++) {
			width += ConflictWithTableModel.values()[index].getWidth();
		}
		return width;
	}
	...
	public static int[] getDefaultOrder() {
		int size = ConflictWithTableModel.values().length;
